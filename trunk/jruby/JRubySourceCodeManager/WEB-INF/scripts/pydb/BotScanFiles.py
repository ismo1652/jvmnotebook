...
def main():
    ...
    if z:                   
        for n in z:
            ...
            #
            # Perform our code specific filters, remove the double //
            # and also remove the 'dump' prefix
            filter_nfilename = n.replace('//', '/')
            filter_nfilename = filter_nfilename[7:]
            
            #
            # Extract the project name
            project_name = filter_nfilename.split('/')[1]
            
            print "%s %s %s %s %s" % (filter_nfilename, fstatus[stat.ST_SIZE], fstatus[stat.ST_MTIME],
                                       filename_ext, project_name)

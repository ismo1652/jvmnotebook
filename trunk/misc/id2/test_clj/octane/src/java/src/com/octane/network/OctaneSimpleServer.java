 ...
 * Simple Test Server 
 */
public class OctaneSimpleServer {
	public static final int DEFAULT_PORT = 9999;

	private int port = 9999;
	private ServerSocket server;

	public OctaneSimpleServer(int port) {
		this.port = port;
	}
	public void runServer() {
		...
		try {		
			while (true) {
				OctaneSimpleServerThread clientThread;
				try {
					...
					clientThread = new OctaneSimpleServerThread(clientSocket);
	...
	public static void main(String[] args) {
		...
		OctaneSimpleServer server = new OctaneSimpleServer(DEFAULT_PORT);
		server.runServer();

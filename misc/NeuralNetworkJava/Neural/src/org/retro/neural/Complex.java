...
// Berlin Brown
//
// $Id: Complex.java,v 1.1.1.1 2004/04/26 13:27:29 bigbinc Exp $
//
//
...
public class Complex extends FieldElement{
	...
	if (val instanceof Rational){
	    return new Complex((Rational)(this.realPart.add(val)), 
			       this.imaginaryPart);
	}
	...
	if (val instanceof Rational){
	    return new Complex((Rational)(this.realPart.multiply(val)), 
                               (Rational)(this.imaginaryPart.multiply(val)));
	}
	Complex mult = (Complex)val;
	Rational newRealPart = 
         (Rational)(this.realPart.multiply(
            mult.getReal())).subtract(this.imaginaryPart.multiply(
               mult.getImaginary()));
	Rational newImaginaryPart = (Rational)(this.realPart.multiply(
           mult.getImaginary())).add(this.imaginaryPart.multiply(
               mult.getReal()));
	return new Complex(newRealPart, newImaginaryPart);
    }

    public FieldElement negate(){
	return new Complex((Rational)realPart.negate(), 
	...
	Rational normalize = (Rational)this.realPart.multiply(
              this.realPart).add(this.imaginaryPart.multiply(
                 this.imaginaryPart));
	return 
          new Complex((Rational)this.realPart.divide(normalize), 

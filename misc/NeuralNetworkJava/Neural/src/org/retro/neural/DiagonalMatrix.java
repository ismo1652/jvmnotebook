...
// Berlin Brown
//
// $Id: DiagonalMatrix.java,v 1.1.1.1 2004/04/26 13:27:31 bigbinc Exp $
//
//
...
class DiagonalMatrix extends Matrix{
	...
	public DiagonalMatrix (FieldElement[] diagElements) throws InvalidOperationException{
	...
	public void setEntry(int rowIndex, int colIndex, FieldElement newEntry) throws InvalidOperationException{
	...
	public FieldElement getDiagElement(int position) throws InvalidOperationException{
		
		int size = this.numOfRows;

		if ((position > size) || (position < 0))
			throw new InvalidOperationException("Tried to get Element at Position " + position + ". Size of Matrix:\n " + size);
		else
	...
	public void setDiagElement(int position, FieldElement elem) throws InvalidOperationException{

		int size = this.numOfRows;
		
		if ((position > size) || (position < 0))
			throw new InvalidOperationException("Tried to set Element at Position\n" + position + ". Size of Matrix:\n " + size);
		else
	...
	private void setRC (int rcIndex, Vector vector) throws InvalidOperationException{
	...
	public DiagonalMatrix add(DiagonalMatrix diagMatrix) throws InvalidOperationException{
		...
		for(int i = 1; i <= tmp.getRows(); i++)
			 tmp.setEntry(i, i, this.getEntry(i,i).add(diagMatrix.getEntry(i,i)));
	...
	public Matrix add(Matrix matrix) throws InvalidOperationException{
		...
		for(int i = 1; i <= tmp.numOfRows; i++)
			tmp.setEntry(i, i, this.getEntry(i,i).add(matrix.getEntry(i,i)));
	...
	public DiagonalMatrix subtract(DiagonalMatrix diagMatrix) throws InvalidOperationException{
		
		//size of each DiagonalMatrix is different
		if(this.numOfCols != diagMatrix.numOfRows)
			throw new InvalidOperationException("Tried to subtract \n" + diagMatrix + "from \n" + this + "No correct format!");

		DiagonalMatrix tmp = new DiagonalMatrix(this.numOfRows, diagMatrix.getEntry(1,1).zero());

			for(int i = 1; i <= tmp.numOfRows; i++)
				tmp.setEntry(i, i, this.getEntry(i,i).subtract(diagMatrix.getEntry(i,i)));
			return tmp;
	...
	public Matrix subtract(Matrix matrix) throws InvalidOperationException{
		...
		for(int i = 1; i <= tmp.numOfRows; i++)
			for(int j = 1; j<= tmp.numOfCols; j++)
				tmp.setEntry(i, j, this.getEntry(i, j).subtract(matrix.getEntry(i, j)));
	...
	public Matrix multiply(Matrix matrix) throws InvalidOperationException{
		...
		for(int i=1; i<=matrix.numOfRows; i++)
			for(int j=1; j<=matrix.numOfCols; j++)
				resultMatrix.setEntry(i,j,this.entries[i-1][i-1].multiply(matrix.getEntry(i,j)));
	 ...
	 */
        ...
        public DiagonalMatrix multiply(DiagonalMatrix diagMatrix) throws InvalidOperationException {

		if(this.numOfCols != diagMatrix.getRows())
			throw new InvalidOperationException("Tried to multiply \n" + this + "and \n" + diagMatrix + "No correct format!");

		DiagonalMatrix result = new DiagonalMatrix(this.getRows(), diagMatrix.getEntry(1,1).zero());

		for (int i = 1 ; i <= diagMatrix.numOfRows ; i++ )
			result.setDiagElement(i , this.entries[i-1][i-1].multiply(diagMatrix.getEntry(i,i)));
	...
	public Matrix copy(){
		...
		DiagonalMatrix result = new DiagonalMatrix(this.getRows(), this.getEntry(1,1).zero());
	...
	public FieldElement det(){
		...
		FieldElement result = this.getEntry(1,1).one();
	...
	public void swapRows(int rowIndex1, int rowIndex2) throws InvalidOperationException{
	...
	public void swapCols(int colIndex1, int colIndex2) throws InvalidOperationException{
	...
	public Matrix inverse() throws InvalidOperationException{
		...
		DiagonalMatrix reverse = new DiagonalMatrix(this.numOfRows, this.getEntry(1,1).zero());
		...
		if (this.contZeroRow())
			...
			for(int i=1; i<=numOfRows; i++)
			    ...
			    reverse.setEntry(i,i,((this.getEntry(1,1).one()).divide(diagonalElements[i-1])));

...
class StackMapTableData {
    ...
    static StackMapTableData getInstance(DataInputStream in, MethodData method)
        ...
        } else if (RuntimeConstants.SAME_FRAME_BOUND <= frameType && frameType < RuntimeConstants.SAME_LOCALS_1_STACK_ITEM_BOUND) {
            // same_locals_1_stack_item_frame
            // read additional single stack element
            return new SameLocals1StackItem(frameType,
                                            (frameType - RuntimeConstants.SAME_FRAME_BOUND),
        ...
        } else if (RuntimeConstants.SAME_FRAME_EXTENDED < frameType
                        && frameType < RuntimeConstants.FULL_FRAME) {
            // append_frame
            return new AppendFrame(frameType, in.readUnsignedShort(),
                                   StackMapData.readTypeArray(in, frameType - RuntimeConstants.SAME_FRAME_EXTENDED, method));

...
;; The following examples and code snippets were taken from the
;; Clojure.org API page.
;; The text is mostly copyrighted to Copyright 2008-2009 Rich Hickey
;;
;; Create Date: 2/24/2009
;; Primary Author: Berlin Brown
;;
;; Tested with Clojure release:  Clojure release 20081217   
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;If supplied, a (set of) public static factory function(s) will be created with the given name, and the same signature(s) as the constructor(s). :state name
;;
...
;; First: define the namespace, imports, and this junit class
(ns api_examples
    ...
    (:gen-class
     ...
     [
      ...
      [ test_proc_time_macro [] void ]
      [ test_safe_try_macro [] void ]
      [ test_example_fn [] void ]
      ]))
...
;;
;; params => positional-params* , or positional-params* & rest-param positional-param => binding-form rest-param => binding-form name => symbol
;;
;; Defines a function
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defn example-fn-test []
  (let [my-func   (fn [] (+ 1 2 3))
        my-func2  (fn [x](+ x 1 2 3))]    
    (my-func)
    (my-func2 4)))
...
(defmacro proc-time [expr]
  `(let [start# (. System (currentTimeMillis))
                ret#     ~expr
                res-t#   (double (- (. System (currentTimeMillis)) start#))
                str-res# (str "Elapsed time: " res-t# " msecs")]
     {:return ret# :time-text str-res# :timed res-t#}))

(defmacro safe-expr [body]
  ;; Another approach for defmacro
  `(try ~body
        (catch Exception ~'e
               (throw (RuntimeException. (str "ERR: Invalid Expression err => " (.getMessage ~'e))))
               nil)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; End of Macro Examples
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defn -test_example_fn [_]
  (Assert/assertEquals 10 (example-fn-test)))

(defn -test_safe_try_macro
  "Test the simple macro"
  [_]
  ;;;;;;
  (safe-expr (+ 1 0))
  (safe-expr (/ 1 0)))

(defn -test_proc_time_macro
  "Test the proc-time macro"
  [_]
  ;;;;;;
  (let [lot-of-data (range 0 1000000)        
        my-func (fn [] (doseq [x lot-of-data] (* x 3)))]        
    (Assert/assertTrue (> ((proc-time (my-func)) :timed) 0))))

(defn -test_hello_world 
  "Test simple hello world call, to make sure our test is working"
  [_]
  (println "Hello World"))

...
jbyte JvmtiTrace::_trace_flags[155];

jint JvmtiTrace::_max_function_index = 154;
...
jvmtiTrace_SetSystemProperty(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              tty->print_cr("JVMTI [%s] %s {  property='%s' value='%s'", curr_thread_name, func_name, property, value);
  }
  err = jvmti_env->SetSystemProperty(property, value);
  if ( err != JVMTI_ERROR_NONE && (trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
      if ((trace_flags & JvmtiTrace::SHOW_IN) == 0) {
          tty->print_cr("JVMTI [%s] %s {  property='%s' value='%s'", curr_thread_name, func_name, property, value);
    }

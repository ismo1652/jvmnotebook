...
// bigbinc@hotmail.com
//
// $Id: Vector.java,v 1.1.1.1 2004/04/26 13:27:37 bigbinc Exp $
//
...
public class Vector{
    ...
    public void setEntry(int index, FieldElement newEntry){
	try{
	...
	return operate(this, anotherVector, new MultiplyOperator(), 
                       "arrayMultiply");
    }
	...
	return operate(this, anotherVector, new SubtractOperator(), 
		       "subtract");
    }
     ...
     * 
     * @return The inner (dot) product of the two Vectors.
     * @param anotherVector
	...
	if (anotherVector == null){
	    throw new InvalidOperationException("Tried to "+
			    "multiply \n"+this+" and \n"+ anotherVector +
	...
	this.setEntry(index1, this.getEntry(index2));
	this.setEntry(index2, tmp);
    }
	...
	for(int i=1; i <= this.length(); i++){
	    this.setEntry(i, fun.apply(this.getEntry(i)));
	}
    }
	...
	for(int i=1; i <= vector.length(); i++){
	    vector.setEntry(i, fun.apply(this.getEntry(i)));
	}
	...
	for(int i=1; i <= this.length(); i++){
	    this.setEntry(i, fun.apply(this.getEntry(i), scalar));
	}
    }
	...
	for(int i=1; i <= vector.length(); i++){
	    vector.setEntry(i, fun.apply(this.getEntry(i), scalar));
	}
	...
	for(int i = 1; i <= this.length(); i++){
	    tmp.setEntry(i, this.getEntry(i));
	}
	return tmp;
    }
	...
	return comparison(anotherVector, new LessThanOrEqualToComparator(), 
			  "LE");
    }
	...
	return comparison(anotherVector, new GreaterThanOrEqualToComparator(),
			  "GE");
    }
	...
	for (int i=1; i<=theMatrix.getCols(); ++i) {
	    result.setEntry(i, this.multiply(theMatrix.getCol(i)));
	}
    ...
    private Vector comparison(Vector anotherVector, FEComparator comp,
			      String compName) {
    ...
    private static Vector operate(Vector vector1, Vector vector2, 
	...
	for (int i=1; i<=vector1.length(); ++i) {
	    vector3.setEntry(i, fun.apply(vector1.getEntry(i), 
	    			          vector2.getEntry(i)));
	}
    ...
    private static Vector operate(Vector vector, FieldElement scalar,
				  DyadicOperator fun) {

	Vector vector2 = vector.copy();

	for (int i=1; i<=vector.length(); ++i) {
	    vector2.setEntry(i, fun.apply(vector.getEntry(i), scalar));
	}
	...
	for (int i=1; i<=this.length(); ++i) {
	    this.setEntry(i, fun.apply(this.getEntry(i), vector.getEntry(i)));
	}
    }

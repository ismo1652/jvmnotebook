...
def get_gc_stats(line):
	...
	if (m and (len(m.groups()) >= 7)):
		...
		sub_m = pattr_sub.match(subgrp_str)
			...
			sub_totalTmp  = sub_m.group(SUB_TOTAL_IDX)
			
			# Reparse for total
			try:							
				pattr_subsub = re.compile('^(\S*)K(.*)$')
				zm = pattr_subsub.match(sub_totalTmp)
				sub_total = zm.group(1)							
			except:
				print "err(2)"
		else:
		...
		return [ gc_iter, gc_new_start,
                         ...
                         sub_end_occ, sub_main_paus_time, sub_total ]
        else:
                if m:

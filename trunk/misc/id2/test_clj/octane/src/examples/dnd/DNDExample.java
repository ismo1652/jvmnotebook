...
public class DNDExample {
			...
			public void dragStart(org.eclipse.swt.dnd.DragSourceEvent event) {
				...
				case COMBO: {
					...
					if (selection.x == selection.y) {
						...
						dragDataText = string.substring(selection.x,
								selection.y);
					}
					break;
				}
				...
				for (int i = 0; i < dragTypes.length; i++) {
					...
					if (dragTypes[i] instanceof URLTransfer
							&& dragDataURL == null) {
						event.doit = false;
					}
	...
	private void createDragWidget(Composite parent) {
		...
		combo.setItems(new String[] { "Toggle Button", "Radio Button",
				"Checkbox", "Canvas", "Label", "List", "Table", "Tree", "Text",
				"StyledText", "Combo" });
		combo.select(LABEL);
	...
	private Control createWidget(int type, Composite parent, String prefix) {
		...
		case STYLED_TEXT: {
			StyledText text = new StyledText(parent, SWT.BORDER | SWT.MULTI
			...
			for (int i = 0; i < 3; i++) {
				...
				for (int j = 0; j < 3; j++) {
					...
					for (int k = 0; k < 3; k++) {
						...
						subsubItem.setText(0, prefix + " name " + i + " " + j
								+ " " + k);
						subsubItem.setText(1, prefix + " value " + i + " " + j
								+ " " + k);
						subsubItem.setText(2, prefix + " description " + i
								+ " " + j + " " + k);
		...
		case COMBO: {
			...
			combo.setItems(new String[] { "Item a", "Item b", "Item c",
					"Item d" });
			return combo;
		}
		default:
	...
	public void open(Display display) {
		...
		itemImage = new Image(display, DNDExample.class
				.getResourceAsStream("openFolder.gif"));
